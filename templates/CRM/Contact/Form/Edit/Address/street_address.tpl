{*
 ...
 | Change field sequence to street_name, street_number, street_unit   |
 | Now uses 4.7 version of template (still usable with 4.4-4.6)       |
...
{if !empty($form.address.$blockId.street_address)}
    ...
    {if $parseStreetAddress eq 1 && ($action eq 1 || $action eq 2)}
        <tr id="addressElements_{$blockId}" class=hiddenElement>
            {* -- START CHANGE: Change name / number order -- *}
            <td>
